...
#
# Config via env vars (defaults in parens) or scripts/gpu/.env:
...
#
# Install via setup_crontab.sh, or add manually (every 15 min):
#   */15 * * * * /path/to/scripts/gpu/gpu_idle_alert.sh >> /tmp/gpu_idle_alert.log 2>&1
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Load .env so cron (minimal environment) still sees SMTP_* and GPU_IDLE_* vars.
...
send_email() {
    python3 "$SCRIPT_DIR/send_email.py" --subject "$1" --body "$2"
}
